    ...
    <div>
                <el-breadcrumb separator="/" class="bread">
                    ...
                    <el-breadcrumb-item>主机</el-breadcrumb-item>
                     <el-breadcrumb-item>主机组: {{this.$route.query.groupid}}</el-breadcrumb-item>
                </el-breadcrumb>

    <div class="tablebox">
     <el-table v-loading="loading" :data="hostData"  style="width: 100%" border fit>
        ...
        <el-table-column   label="所在组" >
          <template slot-scope="scope">
              <!-- 主要是这里修改了 -->
            <span class="table_group" v-for="group in scope.row.groups">{{group.name}}  </span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="主机状态" >
          <template slot-scope="scope">
             <el-button v-if="scope.row.available == 1" size="small" type="success" @click="test">
...
export default {
  ...
  methods: {
    hostDetail(index,row) {
     ...
     getData() {
                axios.get('/monitor/hostlist?groupid='+this.$route.query.groupid).then((res) => {
            ...
            test(){
                //通过这种方式获得变量值
                console.log(this.$route.query.hostgroup)
            }
  },
  mounted() {
    this.getData()
