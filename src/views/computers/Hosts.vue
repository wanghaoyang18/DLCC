<template>
  ...
  <el-card style="margin-left: 20px;margin-top: 20px;margin-right: 20px">
     <el-table v-loading="loading" :data="hostData"  border style="width:100%"  tooltip-effect="dark" >
        ...
        <el-table-column   prop="name" label="主机名称"  sortable></el-table-column>

        <el-table-column   label="所在组" width="280" >
         ...
         <el-table-column label="操作"  >
                    <template slot-scope="scope">
                      ...
                      <el-button size="small" type="warning"
                                 @click="delhost(scope.$index, scope.row)">删除
                      </el-button>
                    </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    delhost(index,row){
        this.$confirm("确定删除吗?"+row.hostid, "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        })
        .then(() => {
            axios.post('/monitor/delhost/',{
              hostid:row.hostid
            }).then(res=>{
              if (res.status==200){
                this.$message({
